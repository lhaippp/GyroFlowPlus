...
def flow_error_image_np(flow_pred, flow_gt):
    diff_sq = np.sum((flow_pred - flow_gt)**2, axis=2, keepdims=True)
    ret = np.sqrt(diff_sq)
    error = (np.minimum(ret, 5) / 5)
    vis = error_map_norm(error)
