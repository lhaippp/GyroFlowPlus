...
def print_overall_test_metrics(manager):
    ...
    for k, v in manager.test_status.items():
        if 'homo' in str(k):
            if 'pck' in str(k):
                print_str_homo_ghof += '{}: {:.4f}  '.format(k, v.avg)
            else:
                _value = float(str(v.avg)[:4])
                print_str_homo_ghof += '{}: {:.2f}  '.format(k, _value)
        else:
            print_str_flow_ghof += '{}: {:.4f}  '.format(k, v.avg)

    manager.logger.info(colored('GHOF: Homography Results: ', 'red', attrs=['bold']))
    manager.logger.info(colored(print_str_homo_ghof, 'green', attrs=['bold']))
